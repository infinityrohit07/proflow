...
export default function Login() {
  ...
  useEffect(() => {
    // Google OAuth callback: only the access token is in the URL.
    // The refresh token was set as an httpOnly cookie by the server callback.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('/api/v1/auth/login', formData, {
        withCredentials: true, // Ensure the httpOnly refreshToken cookie is stored
      });
      if (res.data?.data?.accessToken) {
        // Only the access token goes in localStorage (used for Authorization header).
        // The refresh token is stored securely in an httpOnly cookie by the server.
  ...
  return (
    <div className="min-h-screen bg-[#070a13] flex flex-col md:grid md:grid-cols-2 font-sans relative overflow-hidden">
      ...
      {/* Left Column: Branding & Intro Text */}
      <div className="hidden md:flex flex-col justify-between p-12 lg:p-16 z-10 border-r border-white/5 relative bg-[#090d1a]/20 backdrop-blur-sm">
        ...
        <div className="my-auto max-w-md space-y-6">
          <h1 className="text-4xl lg:text-5xl font-extrabold text-white leading-tight tracking-tight">
            Supercharge your team's project flow.
          </h1>
          <p className="text-slate-400 text-base leading-relaxed">
            Log in to manage tasks visually, broadcast updates in real time, and query project diagnostics with your embedded AI companion.
          </p>
          <div className="flex items-center gap-4 text-xs text-slate-500 font-semibold uppercase tracking-wider pt-2">
        ...
        <div className="mx-auto w-full max-w-md">
          ...
          <div className="glass-card py-8 px-6 sm:px-10 rounded-2xl border border-white/5 shadow-2xl">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Email or Username</label>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 px-4 bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-7-0 text-white font-semibold rounded-xl shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 transition-all focus:outline-none cursor-pointer flex items-center justify-center gap-2 border border-white/10 active:scale-[0.98]"
              >
                {loading ? 'Signing in...' : (
                  <>
